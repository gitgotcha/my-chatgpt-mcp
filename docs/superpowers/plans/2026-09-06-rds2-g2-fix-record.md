...
> **【更正 2026-09-07】** 上一句是过早结论，已被补充复审推翻：scope 内仍存在三个实质缺口
> （F1 累计状态随历史增长卡死 scope、F2 存储故障未计数未退避、F3 重放缺必填范围与结构校验），
> 详见 `2026-09-06-rds2-g2-followup-review.md` 与修订方案 `2026-09-06-rds2-g2-f1f3-plan.md`（§9 执行记录）。
> 「全部可达路径均已覆盖」仅在**当时已知的验收口径**下成立，不覆盖本轮探针新增的三类要求。

## 1. 提交清单

下表**严格按 git 历史由旧到新排列**，覆盖基线 `218ff8f`（本轮之前的最后一个提交，未入表）
之后的全部实质提交。核对方式：

```
git log --oneline 218ff8f..HEAD
```

其结果应为下表 25 行，**后随若干条仅修订本文件的记录提交** —— 记录提交无法在自身内容里
列出自己的 SHA，故这些提交不在表内（当前为 `4b6d338` 及之后同类提交）。
