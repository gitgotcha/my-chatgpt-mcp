...
export function createArtifactSyncHandler(env: SyncEnvironment, repository: D1ArtifactRepository, destination: ArtifactDestination) {
  return async (request: Request): Promise<Response> => {
    ...
    const bytes = await repository.loadContent(stored.jobId); if (!bytes) { await repository.seal(stored.jobId, owner, "artifact_staging_missing"); return response(489, { "Upstash-NonRetryable-Error": "true" }); }
    const outcome = await destination.syncArtifact(stored.artifact, bytes);
