...
async function safeReason(response: Response): Promise<SafeDriveReason | undefined> {
  try {
    const body = await response.json() as { error?: { errors?: Array<{ reason?: unknown }> } };
    const reason = body.error?.errors?.[0]?.reason;
    return reason === "storageQuotaExceeded" || reason === "insufficientFilePermissions" || reason === "accessNotConfigured" ? reason : undefined;
  } catch {
    return undefined;
  }
}
...
function problem(error: unknown): SyncOutcome {
  ...
  if (status !== undefined && status >= 400 && status < 500) console.warn("Google Drive request rejected", { status, operation: isRecord(error) && typeof error.operation === "string" ? error.operation : undefined, reason: isRecord(error) && typeof error.reason === "string" ? error.reason : undefined });
