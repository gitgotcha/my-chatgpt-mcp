...
async function bodyHash(value: string): Promise<string> {
...
export async function verifyQStashSignature(signature: string | null, rawBody: string, url: string | undefined, keys: string[]): Promise<boolean> {
  ...
  try {
    ...
    const expectedBodyHash = await bodyHash(rawBody);
    const claimBodyHash = typeof claims.body === "string" ? claims.body.replace(/=+$/, "") : "";
    if (header.alg !== "HS256" || claims.iss !== "Upstash" || claims.sub !== url || typeof claims.exp !== "number" || (typeof claims.nbf === "number" && claims.nbf > Date.now() / 1000) || claims.exp <= Date.now() / 1000 || claimBodyHash !== expectedBodyHash.replace(/=+$/, "")) return false;
